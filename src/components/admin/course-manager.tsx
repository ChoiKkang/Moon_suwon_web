  ...
  return (
    <div className="grid grid-cols-1 gap-6 xl:grid-cols-[340px_minmax(0,1fr)]">
      ...
      <section className="rounded-3xl border border-[#ffd700]/20 bg-[#171f33]/90 p-5 md:p-7">
        <div className="flex flex-col justify-between gap-4 border-b border-[#3e495d]/30 pb-5 md:flex-row md:items-center"><div><p className="text-xs font-black uppercase tracking-[0.2em] text-[#ffd700]">Course Editor</p><h2 className="mt-2 text-2xl font-black text-white">{selected ? selected.copy.heroTitle || selected.slug : '새 코스'}</h2></div><div className="flex flex-wrap items-center gap-3"><AdminStatusBadge label={draft.isPublished ? 'PUBLIC' : 'DRAFT'} tone={draft.isPublished ? 'success' : 'muted'} />{selected?.automationSource ? <AdminStatusBadge label="자동 초안" tone="info" /> : null}<button type="button" disabled={isPending} onClick={generateDrafts} className="inline-flex items-center gap-2 rounded-xl border border-[#ffd700]/40 px-4 py-2.5 text-sm font-black text-[#ffd700] disabled:opacity-50"><Sparkles className="h-4 w-4" />{isPending ? '생성 중…' : '자동 초안 생성'}</button><button type="button" disabled={isPending} onClick={save} className="inline-flex items-center gap-2 rounded-xl bg-[#ffd700] px-4 py-2.5 text-sm font-black text-[#3a3000] disabled:opacity-50"><Save className="h-4 w-4" />{isPending ? '저장 중…' : '저장'}</button></div></div>
        ...
        {selected?.automationSource ? <section className="mt-7 rounded-2xl border border-sky-300/20 bg-sky-300/5 p-4"><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[0.18em] text-sky-200">Generation Evidence</p><p className="mt-1 text-sm font-bold text-white">검수 후 공개 · 자동 생성 provenance</p></div><AdminStatusBadge label={String(selected.automationMetadata.distance_kind ?? 'straight_line_estimate') === 'straight_line_estimate' ? '직선거리 추정' : '경로 기반'} tone="warning" /></div><p className="mt-3 text-xs leading-relaxed text-sky-100/80">자동 코스의 거리는 실제 도보 경로가 아닌 좌표 직선거리 추정치일 수 있습니다. 장소별 근거를 확인한 뒤 공개하세요.</p><div className="mt-4 space-y-2">{Array.isArray(selected.automationMetadata.evidence) ? (selected.automationMetadata.evidence as Array<{ placeId?: string; reasons?: string[] }>).map((item) => <div key={item.placeId} className="rounded-xl bg-[#0b1326]/70 p-3 text-xs"><p className="font-bold text-white">{places.find((place) => place.id === item.placeId)?.displayName ?? item.placeId}</p><p className="mt-1 text-[#d0c6ab]">{item.reasons?.join(' · ') || '근거 없음'}</p></div>) : null}</div></section> : null}
