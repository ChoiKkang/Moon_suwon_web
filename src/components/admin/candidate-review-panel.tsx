      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
            ...
            <tbody className="divide-y divide-[#3e495d]/20">
              {filtered.map((candidate) => (
                <tr key={candidate.placeId} onClick={() => select(candidate)} className={`cursor-pointer transition hover:bg-[#0b1326]/60 ${selectedId === candidate.placeId ? 'bg-[#ffd700]/5' : ''}`}>
                  <td className="px-3 py-4"><p className="font-bold text-white">{candidate.displayName}</p><p className="mt-1 text-[11px] text-[#8f9bb3]">{candidate.ktoContentId ?? 'KTO ID 없음'} · {candidate.category ?? '분류 없음'}</p></td>
        ...
        {selected ? (
          <aside className="rounded-2xl border border-[#ffd700]/20 bg-[#0b1326]/70 p-5" aria-label={`${selected.displayName} 후보 검수`}>
            <div className="flex items-start justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[0.18em] text-[#ffd700]">Review Detail</p><h3 className="mt-2 text-xl font-black text-white">{selected.displayName}</h3><p className="mt-1 text-xs text-[#8f9bb3]">{selected.addressFull ?? '주소 정보 없음'}</p></div><button type="button" aria-label="후보 상세 닫기" onClick={() => setSelectedId(null)} className="rounded-lg p-2 text-[#8f9bb3] hover:bg-[#171f33] hover:text-white"><X className="h-4 w-4" /></button></div>
