...
export default async function AdminPlacesPage({
  ...
  const [{ data, error }, params] = await Promise.all([getAdminPlaces(), searchParams]);
  ...
  return (
    <main className="relative z-10 mx-auto w-full max-w-[1440px] flex-1 p-6 md:p-12 xl:p-16">
      ...
      {error ? (
        <div className="rounded-3xl border border-amber-400/30 bg-amber-400/10 p-6 text-sm text-amber-100">장소 운영 데이터를 불러오지 못했습니다. {error}</div>
      ) : (
